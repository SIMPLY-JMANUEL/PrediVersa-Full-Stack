      ...
      <div className="settings-panel-content" style={{ maxWidth: 540 }}>
        ...
        <div className="settings-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          <div style={{ gridColumn: '1 / span 2', textAlign: 'center' }}>
            ...
            {profile.photo && <img src={profile.photo} alt="Perfil" className="settings-profile-img" />}
          </div>
          <label>Nombre
          ...
          <label>Tipo documento
            <input type="text" name="documentType" value={profile.documentType || ''} onChange={handleChange} />
          </label>
          <label>Número documento
            <input type="text" name="documentNumber" value={profile.documentNumber || ''} readOnly style={{ background: '#f3f6fa' }} />
          </label>
          <label>Correo
            <input type="email" name="email" value={profile.email || ''} readOnly style={{ background: '#f3f6fa' }} />
          </label>
          <label>Teléfono
            <input type="text" name="phone" value={profile.phone || ''} onChange={handleChange} maxLength={20} />
          </label>
          <label>Dirección
            <input type="text" name="address" value={profile.address || ''} onChange={handleChange} maxLength={100} />
          </label>
        ...
        <div className="settings-panel-actions">
          <button onClick={handleSave} className="settings-save-btn">Guardar</button>
          <button onClick={onClose} className="settings-cancel-btn">Cancelar</button>
        </div>
      </div>
    </div>
  );
